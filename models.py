...
class UNet(nn.Module):
    def __init__(self, num_channels=3, num_classes=1):
        ...
        self.final = nn.Sequential(nn.Conv2d(num_feat[0],
                                             ...
                                             kernel_size=1),
                                   nn.Sigmoid())
    def get_ssim_loss(self,tensor):
        N, D, _, _ = tensor.shape
        temp = [i for i in range(D)]
        shuffle(temp)
        temp = np.array(temp)
        return self.ssim(tensor[:,temp[:D//2]],tensor[:,temp[D//2:]])
    ...
    def forward(self, inputs, return_features=False):
        ...
        ssim_loss = torch.sqrt(self.get_ssim_loss(down3_feat)**2 + self.get_ssim_loss(down4_feat)**2 + self.get_ssim_loss(up2_feat)**2 + self.get_ssim_loss(up1_feat)**2)

        return outputs,ssim_loss
...
class UNetSmall(nn.Module):
    ...
    def forward(self, inputs, return_features=False):
        ...
        ssim_loss = torch.sqrt(self.get_ssim_loss(down2_feat)**2 + self.get_ssim_loss(down3_feat)**2 + self.get_ssim_loss(up2_feat)**2 + self.get_ssim_loss(up1_feat)**2)
        return outputs, ssim_loss
