...
class NewCRFDepth(nn.Module):
    ...
    def upsample_mask(self, disp, mask):
        ...
        up_disp = up_disp.permute(0, 6, 4, 2, 5, 3)
    ...
    def forward(self, imgs):
        ...
        depth = self.softmax(d1)
        ssim_loss = torch.sqrt(self.get_ssim_loss(e3)**2 + self.get_ssim_loss(e2)**2 + self.get_ssim_loss(e1)**2)
        return depth, ssim_loss
...
class DispHead(nn.Module):
    def __init__(self, input_dim=100):
        ...
        self.conv1 = nn.Conv2d(input_dim, 6, 3, padding=1)
